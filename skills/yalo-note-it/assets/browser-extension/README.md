...
5. 角标显示 `OK` 后，记录包位于 `D:\MyData\yalo-note\sessions\chatgpt\<conversation-id>\`：
...
这个状态表示插件还保存着目录句柄，但浏览器当前没有向该句柄授予写权限。常见于扩展被重新加载或浏览器重新启动。0.9.1 起，直接点击“Yalo note it”即可通过浏览器授权框恢复访问，无需重新选择目录。
